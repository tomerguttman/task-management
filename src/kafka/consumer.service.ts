...
export class ConsumerService implements OnApplicationShutdown {
  ...
  async consume(topic: string, configuration: ConsumerRunConfig): Promise<void> {
    ...
    this.logger.log(`Consumer for topic ${topic} started with group ID ${groupId}`);
  }

  async onApplicationShutdown(): Promise<void> {
    const disconnectPromises: Promise<void>[] =
      this.consumers.map((consumer: Consumer): Promise<void> => {
